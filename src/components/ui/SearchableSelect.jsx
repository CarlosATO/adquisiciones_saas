...
export default function SearchableSelect({ options, value, onChange, placeholder = "Buscar...", className = "" }) {
  ...
  return (
    <div ref={wrapperRef} className={`relative ${className}`}>
      {/* Botón Principal */}
      <div
        className="flex items-center justify-between border border-slate-300 hover:border-indigo-400 rounded-lg px-3 py-2 bg-white cursor-pointer transition-colors"
      ...
      {isOpen && (
        <div className="absolute z-[100] w-full mt-1 bg-white border border-slate-200 rounded-lg shadow-xl max-h-60 overflow-y-auto">
          ...
          <div className="py-1">
            {filteredOptions.length === 0 ? (
              ...
              filteredOptions.map(opt => (
                <div
                  key={opt.value}
                  className={`px-3 py-2 text-sm cursor-pointer flex items-center justify-between transition-colors ${value === opt.value ? 'bg-indigo-50 text-indigo-700 font-bold' : 'text-slate-700 hover:bg-slate-50'}`}
                  onClick={() => {
                    onChange(opt.value);
                    setIsOpen(false);
                    setSearchTerm("");
