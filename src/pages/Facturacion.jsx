...
import { FileText, Search, ChevronRight, X, Eye } from 'lucide-react';
...
export default function Facturacion() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // 1. Órdenes calificadas (PARTIAL o RECEIVED)
      ...
      if (!orders?.length) { setReceipts([]); setLoading(false); return; }

      const poIds    = orders.map(o => o.id);
      const poMap    = Object.fromEntries(orders.map(o => [o.id, o]));

      // 2. Recepciones para esas OC
      ...
      if (!receiptData?.length) { setReceipts([]); setLoading(false); return; }
      ...
      const [
        ...
        supabase.from('purchase_order_items')
          .select('po_id, product_id, unit_cost')
          .in('po_id', poIds),
        supabase.from('expenses')
          .select('id, po_id, receipt_id, document_number, amount, expense_date, due_date, status')
          .in('po_id', poIds),
        supabase.from('suppliers')
          .select('id, business_name, name')
          .in('id', [...new Set(orders.map(o => o.supplier_id).filter(Boolean))]),
      ]);

      const supplierMap = Object.fromEntries((suppliers || []).map(s => [s.id, s]));

      // 4. Enriquecer cada recepción
      const enriched = receiptData.map(receipt => {
        const po       = poMap[receipt.po_id];
        const supplier = supplierMap[receipt.supplier_id] || supplierMap[po?.supplier_id];

        // Monto: movimientos de esta recepción × costo unitario de la OC
        ...
        // Factura vinculada (por receipt_id o legacy por document_number+po_id)
        const linkedExpense = (expenses || []).find(e =>
          e.receipt_id === receipt.id ||
          (receipt.document_type === 'FACTURA' &&
            e.po_id === receipt.po_id &&
            e.document_number === receipt.document_number)
        );

        // Si fue recepcionado con FACTURA → ya está facturado automáticamente
        const isInvoiced    = receipt.document_type === 'FACTURA' || !!linkedExpense;
        const invoiceNumber = receipt.document_type === 'FACTURA'
          ? receipt.document_number
          : linkedExpense?.document_number;
                  ...
                  return (
                    <tr key={r.id} className={`transition-colors ${r.isInvoiced ? 'bg-slate-50/40' : 'hover:bg-indigo-50/30'}`}>
                      <td className="px-4 py-2 font-black text-indigo-600 font-mono">
                        #{String(r.po?.po_number || '').padStart(4, '0')}
                      </td>
